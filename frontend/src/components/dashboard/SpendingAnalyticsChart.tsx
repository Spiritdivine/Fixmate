...
export const SpendingAnalyticsChart: React.FC<SpendingAnalyticsChartProps> = ({ data }) => {
  return (
    <div className="bg-white p-6 rounded-[24px] shadow-sm h-full flex flex-col relative">
      <h3 className="text-base font-semibold text-slate-800 mb-6">Project Analytics</h3>
      
      <div className="flex-1 w-full min-h-[160px] relative mt-2">
        {/* Tooltip positioned absolutely for the 'T' light bar - mimicking the design */}
        ...
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={data} margin={{ top: 10, right: 10, left: 10, bottom: 0 }} barSize={32}>
            <defs>
              <pattern id="client-striped-pattern" patternUnits="userSpaceOnUse" width="8" height="8" patternTransform="rotate(45)">
            ...
            <Bar dataKey="value" radius={[20, 20, 20, 20]}>
              {data.map((entry, index) => {
                let fill = 'url(#client-striped-pattern)';
